 ...
 */
  ...
  if(isset($_POST['tweet'])){
    ...
    $place = $getFromU->checkinput($_POST['place']);
    $season = $getFromU->checkinput($_POST['season']);
    ...
    if(!empty($status) or !empty($_FILES['file']['name'][0])){
      if(!empty($_FILES['file']['name'][0])){
        $image = $getFromU->uploadImage($_FILES['file']);
      }

      if(strlen($status) > 140 ){
        $error = "The text of your post is too long";
      }
         $tweet_id = $getFromU->create('tweets', array('status' => $status,'place' =>$place,'season'=>$season,'image' =>$image ,'postBy' => $user_id,  'postedOn' => date('Y-m-d H:i:s')));
       preg_match_all("/#+([a-zA-Z0-9_]+)/i", $status, $hashtag);

      if(!empty($hashtag)){
        $getFromT->addTrend($status);
      }
      $getFromT->addMention($status, $user_id, $tweet_id,$image);
    }else{
		...
		<div class="nav-left">
			<ul>
        	  ...
        	  <?php if($getFromU->loggedIn() === true){ ?>
				<li><a href="./notification.php"><i class="fa fa-bell" aria-hidden="true"></i>Notifications</a></li>
				
        	  <?php } ?>
        	  <li><a href="../tms/package-list.php"><i class="fa fa-briefcase" aria-hidden="true"></i></i>Packages</a></li>
        	   <li><a href="../tms/tour-history.php"><i class="fa fa-plane" aria-hidden="true"></i>tour history</a></li>
        	   <a><li id="messagePopup"><i class="fa fa-envelope" aria-hidden="true"></i>Messages</a>s</li>
				...
				<div class="drop-wrap">
					<div class="drop-inner">
						<ul>
							...
							<li><a href="../includes/index.php">Log out</a></li>
						</ul>
					</div>
				</div>
				</li>
				...
				<div class="tweet-wrap">
					<div class="tweet-inner">
						 ...
						 <form method="post" enctype="multipart/form-data">
						 	<ul>
						 		<li>
						 		<textarea class="place"  maxlength="141" name="place" placeholder="PLACE" rows="4" cols="50"></textarea>
						 		</li>
						 		<li>
						 		<textarea class="season"  maxlength="141" name="season" placeholder="SEASON!" rows="4" cols="50"></textarea>
						 		</li>
						 		<li>
						 		<textarea class="status"  maxlength="141" name="status" placeholder="TELL US MORE!" rows="4" cols="50"></textarea>
						 		</li>
